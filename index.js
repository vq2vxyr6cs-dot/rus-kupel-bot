...
function buildSummary(ctx) {
  ...
  return (
    ...
    '👉 Чтобы подтвердить бронь, нажмите «Забронировать».\n' +
    '✏️ Чтобы изменить данные, нажмите «Изменить».'
  );
}
...
bot.hears('🟢 Забронировать', async (ctx) => {
  ...
  if (step === 'done') {
    ...
    // Сбрасываем, чтобы следующее нажатие начало новую бронь
    resetBooking(ctx);
    ctx.session.booking.step = 'start';
    return;
  }
...
bot.hears('✏️ Изменить', (ctx) => {
  resetBooking(ctx);
  ctx.session.booking.step = 'bath';
  return ctx.reply('Изменим бронь. Выберите баню:', bathKeyboard());
});
