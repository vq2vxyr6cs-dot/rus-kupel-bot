...
function resetBooking(ctx) {
  ctx.session.booking = {
    bath: null,      // 'tsar' | 'bogatyr'
    date: null,      // строка даты
    time: null,      // строка времени
    hours: null,     // число или 'more'
    kupel: null,     // true | false | null
    venik: null,     // 'oak' | 'birch' | 'none'
...
function buildSummary(ctx) {
  ...
  const bathName =
    b.bath === 'tsar' ? 'Царь баня' :
    b.bath === 'bogatyr' ? 'Богатырская баня' :
    '—';

  const kupelText =
    b.bath === 'bogatyr'
      ? (b.kupel ? 'с купелью' : 'без купели')
      : '—';

  let venikText = '—';
  if (b.venik === 'oak') venikText = 'дубовый';
  if (b.venik === 'birch') venikText = 'берёзовый';
  if (b.venik === 'none') venikText = 'без веника';
  ...
  return (
    ...
    `Дата: ${b.date || '—'}\n` +
    `Время начала: ${b.time || '—'}\n` +
    `Длительность: ${b.hours === 'more' ? 'более 4 часов' : (b.hours ? b.hours + ' ч.' : '—')}\n` +
    (b.bath === 'bogatyr' ? `Купель: ${kupelText}\n` : '') +
    `Веник: ${venikText}\n\n` +
    'Если всё верно — администратор свяжется с вами для подтверждения. ' +
    'Чтобы начать заново, нажмите «🟢 Забронировать».'
  );
}
